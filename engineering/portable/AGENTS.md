...
Before routing, apply the portable `backpack-enhance-prompt` skill automatically only
...
explicitly available, preserves context, and the prompt clearly says Backpack Engineering will
...
## Visible Backpack Engineering context

Never expose hidden chain-of-thought or pretend to reveal private reasoning.
Instead, make the active Backpack Engineering workflow observable in the conversation.
